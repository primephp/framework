...
class WideImage {
    ...
    static function loadFromFile($uri) {
        ...
        if (!self::isValidImageHandle($handle))
            throw new WideImage_InvalidImageSourceException("Arquivo '{$uri}' parece ser uma fonte de imagem inválida.");
    ...
    static function loadFromString($string) {
        if (strlen($string) < 128)
            throw new WideImage_InvalidImageSourceException("String não contém dados de imagem.");
        ...
        if (!self::isValidImageHandle($handle))
            throw new WideImage_InvalidImageSourceException("String não contém dados de imagem válido.");
    ...
    static function loadFromUpload($field_name, $index = null) {
        if (!array_key_exists($field_name, $_FILES))
            throw new WideImage_InvalidImageSourceException("Upload de campo '{$field_name}' não existe.");
        ...
        if (!file_exists($filename))
            throw new WideImage_InvalidImageSourceException("Arquivo enviado não existe.");
    ...
    static function assertValidImageHandle($handle) {
        if (!self::isValidImageHandle($handle))
            throw new WideImage_InvalidImageHandleException("{$handle} não é um identificador de imagem válido.");
    }
